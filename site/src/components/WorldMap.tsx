...
const MemoGeography = memo(function MemoGeo({
	...
	geo: Parameters<Parameters<typeof Geographies>[0]['children']>[0]['geographies'][0]
	fill: string
...
export function WorldMap({
	...
	return (
		<div className="relative w-full overflow-hidden rounded-xl border border-border bg-bg-card">
			<ComposableMap
					...
					<Geographies geography={GEO_URL}>
						{({ geographies }) =>
							geographies.map((geo) => {
								...
								const iso2 =
									ISO_NUMERIC_TO_ISO2[numericId] ||
									geo.properties?.['ISO_A2'] ||
									''
								const country = countryMap.get(iso2)
								const fill = country
									? getContinentColor(country.continent)
									: '#1e1e24'
								return (
									<MemoGeography
										key={geo.rsmKey}
			...
			{tooltip ? (
				...
				>
					<div className="flex items-center gap-2">
						...
						<span className="text-sm font-semibold text-text">{tooltip.name}</span>
					</div>
					<div className="mt-1 text-xs text-text-muted">
						Capital: {tooltip.capital}
					</div>
					<div className="text-xs text-text-muted">
						Population: {formatCompact(tooltip.population)}
					</div>
				</div>
			) : null}
		</div>
