...
import {
	...
	searchCountryCitiesPaginated,
	searchCountriesPaginated,
	searchStatesPaginated
} from './db/queries'
import { openApiSpec } from './openapi'
import type { Location, PaginatedResponse, SearchResult } from './types'
...
app.get('/postman.json', (c) =>
	c.json(
		createPostmanCollection(
			openApiSpec(getSiteConfig(c.env, c.req.url)),
			'Geocoded API v1'
		),
		200,
		{
...
app.get('/countries/:country/cities', async (c) => {
	const db = c.env.GEO_DB
	...
	const q = c.req.query('q')?.trim()
	if (q) {
		const { rows, total } = await searchCountryCitiesPaginated(
			db,
			countryCode,
			q,
			page.limit,
	...
	const { rows, total } = await getCountryCitiesPaginated(
		db,
		countryCode,
		page.limit,
		page.offset
